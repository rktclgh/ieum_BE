...
public class ChatRoomLifecycleService implements ChatRoomLifecycle {
	...
	public Long createGroupRoom(Long meetingId, Long hostUserId) {
		...
		ChatRoom room = chatRoomRepository.saveAndFlush(ChatRoom.group(meetingId));
		chatMemberRepository.save(ChatMember.join(room, host));
		return room.getId();
	}
	...
	public Long getOrCreateQuestionRoom(Long questionId, Long firstUserId, Long secondUserId) {
		...
		restoreMember(room, firstUser, chatMemberRepository.findByRoom_Id(room.getId()));
		restoreMember(room, secondUser, chatMemberRepository.findByRoom_Id(room.getId()));
		return room.getId();
	}
