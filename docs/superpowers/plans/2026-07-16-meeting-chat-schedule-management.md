...
**Architecture:** 기존 `meeting_schedules`와 `reports`를 additive하게 확장한다. 일정 CRUD는 `MeetingService`가 소유하고, 신고는 기존 `ReportService`와 관리자 신고함을 재사용한다. 새 화면 권한은 서버가 `MeetingScheduleItem`에 capability로 내려준다.
...
- [ ] **Task 1 — Lock the schedule wire/domain contract with focused tests.**
  - Add/extend `MeetingScheduleTest` and `MeetingServiceTest` cases for title/location mutation, multiple future one-time schedules, own edit/delete, host deletion of another member's schedule, and recurring create/update rejection.
...
- [ ] **Task 2 — Implement one-time schedule CRUD and server capabilities.**
  ...
  - In `MeetingService`, serialize mutation on the meeting row, preserve joined/kicked validation, remove the single-active-schedule 409 for one-time meetings, reject manual creation/update for recurring meetings, and re-compute `meetings.meeting_at` after each mutation.
  ...
  - Build `canEdit/canDelete/canReport` at read time from the same predicates used by mutations.

- [ ] **Task 3 — Add schedule report persistence and manual review flow.**
  - First add focused `ReportServiceTest`/`ReportContextSnapshotFactoryTest` cases for a joined user reporting another member's schedule, rejected self/non-member/kicked cases, immutable snapshot content, and `aiReviewState=cancelled`.
...
- [ ] **Task 6 — Verify the module and record the as-built contract.**
  - Run only the affected meeting/report/admin focused Gradle tests and `./gradlew compileKotlin` (or the repository's equivalent compile task).
