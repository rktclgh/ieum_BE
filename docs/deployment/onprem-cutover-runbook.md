...
7. write activation phases (`INSTALLED`, `MIGRATION_STARTED`, `MIGRATION_SUCCEEDED`, `SERVICES_HEALTHY`, `NGINX_STAGED`, `COMMIT_PENDING`, `ACTIVE`) to the root-only journal, together with monotonic `MIGRATION_STARTED` and `MIGRATION_SUCCEEDED` markers. If migration has started and any later step fails, record `MANUAL_INTERVENTION` and leave the write fence enabled; do not silently roll back over an uncertain schema. A first release with no `current` pointer may retry its runtime/public gates without rerunning migrations only when `MIGRATION_SUCCEEDED=true`; a missing legacy marker is not success evidence and requires fresh root-operated `db-verify` parity evidence before reconciliation;
