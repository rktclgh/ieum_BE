...
case "${@: -1}" in
  https://ieum.rktclgh.site/api/places/search)
    [[ "${FAKE_PUBLIC_SMOKE:-up}" == up ]] && { printf '{}\n'; exit 0; }
    exit 22
    ;;
esac
...
run_helper() {
  ...
  FAKE_PUBLIC_SMOKE="${FAKE_PUBLIC_SMOKE:-up}" \
...
[[ "$(grep -Fc -- '--retry 5 --retry-delay 1 --retry-all-errors --noproxy *' "$TMP_DIR/curl.log")" -eq 2 ]] \
  || { printf 'origin smokes must retry after nginx reload\n' >&2; exit 1; }
